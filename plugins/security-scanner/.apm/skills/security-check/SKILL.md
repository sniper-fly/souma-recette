---
name: security-check
description: "リポジトリ全体のマルウェアスキャンを実行する。「マルウェアスキャンして」「セキュリティチェックして」「security-check」等のリクエスト、または `/security-check [--path <path>]` の明示的な呼び出しに対応する。"
allowed_tools: Bash, Glob, Task, Read, Write
---
...
1. Taskツール(リポジトリ探索が得意な汎用サブエージェント種別、例: Claude Codeの`Explore`)を呼び出してリポジトリ構造を分析:
...
Phase 2で得られた各グループに対して、`security-review` スキルを**並列で**使用します。

**重要**: 複数のTaskを1つのメッセージで並列呼び出しすること。各Taskには汎用的なサブエージェント種別を指定し、プロンプトで「`security-review` スキルを使って以下のファイル群をスキャンしてください」と明示的に指示する。

```
# 複数のTaskを同時に呼び出し
Task(汎用サブエージェント):
  prompt: |
    security-review スキルを使って以下をスキャンしてください。
...
4. **漏れがある場合**は追加でTaskを呼び出し、`security-review` スキルを使わせる:
